...
One of the most powerful aspects of `Template` is its **declarative nature**. Instead of manually manipulating the DOM, you declare _what_ the UI should look like for a given state, and `Typed` handles the _how_.
...
```ts
// Recovering locally
const safeHandler = EventHandler.catchCause(
  riskyHandler,
  (cause) => Effect.sync(() => console.error("Caught error:", cause))
)
```
...
> **Thought Experiment:** Imagine if keyboard events were **pull-based**. The browser would have to ask you 60 times a second: _"Did you press 'A' yet? How about now? Now?"_
>
...
```ts
import { RenderPriority, RenderQueue } from "effect/typed/template/RenderQueue"
...
const program = Effect.gen(function*() {
  ...
  const todoList = many(
    ...
    (todoRef, key) =>
      // Render function receives RefSubject
      html`<li>
        ${RefSubject.map(todoRef, (todo) => todo.text)}
        <button onclick=${
        RefSubject.update(todoRef, (todo) => ({ ...todo, completed: !todo.completed }))
      }>Toggle</button>
      </li>`
  )
...
const Button = (label: string, onClick: () => Effect.Effect<void>) => html`<button onclick=${onClick}>${label}</button>`
...
const Form = Fx.gen(function*() {
  ...
  return html`<form onsubmit=${handleSubmit}>
    <input
      .value=${name}
      oninput=${
    EventHandler.make((ev: InputEvent & { target: HTMLInputElement }) => RefSubject.set(name, ev.target.value))
  }
      placeholder="Name"
    />
    <input
      .value=${email}
      oninput=${
    EventHandler.make((ev: InputEvent & { target: HTMLInputElement }) => RefSubject.set(email, ev.target.value))
  }
      placeholder="Email"
    />
    <button type="submit">Submit</button>
...
const TodoApp = Fx.gen(function*() {
  ...
  return html`<div>
    <input
      .value=${newTodoText}
      oninput=${
    EventHandler.make((ev: InputEvent & { target: HTMLInputElement }) => RefSubject.set(newTodoText, ev.target.value))
  }
      onkeydown=${
    EventHandler.make((ev: KeyboardEvent & { target: HTMLInputElement }) =>
...
**React:**
Components are functions that run _every time_ render happens.
...
This means you define your data flow graph _once_, and values flow through it forever. You rarely need to worry about "stale closures" or dependency arrays because the function doesn't re-run.
...
- **Persistent Rows:** If an item's data changes, the component function is _not_ re-executed. Instead, the `todoRef` emits a new value, updating only the tiny part of the DOM that changed.
...
**React:**
JSX is not valid JavaScript. You effectively _must_ use a bundler (Vite, Webpack) and a compiler (Babel, SWC) to develop.
...
- **The Typed Way:** Reactivity is **explicit**. You compose streams using `map`, `combine`, and `flatMap`. Data flow is a visible, static graph constructed once.
  - _Benefit:_ No "magic". You can trace exactly where data comes from. You can pass streams around as first-class values without worrying about "reactive contexts".
...
- **Recovery:** You can use standard Effect error recovery (`catchAll`, `retry`, `orElse`) to handle failures gracefully _at the granularity you choose_.
